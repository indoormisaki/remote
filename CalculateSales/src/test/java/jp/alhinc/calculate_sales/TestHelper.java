...
public final class TestHelper {
     ...
     * </pre>
     *
     * @param obj 呼び出したいprivateなインスタンスメソッドを持つオブジェクト
     * @param methodName 呼び出したいメソッド名
     * @param args 呼び出したいメソッドの引数
     * @return 実行結果
     */
     ...
     * </pre>
     *
     * @param klass 呼び出したいprivateなクラスメソッドを持つクラス
     * @param methodName 呼び出したいメソッド名
     * @param args 呼び出したいメソッドの引数
     * @return 実行結果
     */
     ...
     *
     * @param klass 検索対象クラス
     * @param methodName 実行対象メソッド名
     * @param args 実引数（可変長）
     * @return 実行対象メソッド
     * @throws NoSuchMethodException 合致するメソッドが存在しない場合
     */
    private static Method getInvokeTarget(Class<?> klass, String methodName,
            Object... args) throws NoSuchMethodException {
        Method[] methods = klass.getDeclaredMethods();
        for (Method method : methods) {
            Class<?>[] paramTypes = method.getParameterTypes();
            if (method.getName().equals(methodName)
                    && matchesParamTypes(paramTypes, args)) {
                return method;
            }
        }
     ...
     * 仮引数と実引数の型を比較する
     *
     * @param paramTypes 仮引数の型の配列
     * @param args 実引数の配列
     * @return 比較結果（T：すべて一致、F：一致しない）
     */
    private static boolean matchesParamTypes(Class<?>[] paramTypes,
            Object[] args) {
        if (paramTypes.length != args.length) {
            return false;
        }
        for (int i = 0; i < paramTypes.length; i++) {
            if (args[i] != null && !convertPrimitiveClass(paramTypes[i])
                    .isInstance(args[i])) {
                return false;
            }
        }
